...
class PIRServer:
    ...
    def recv_chat_msg_client(self, msg):
        ...
        # Store the message
        if list(self.chat_messages.keys()) == []:
            last_index = -1
        else:
            last_index = list(self.chat_messages.keys())[-1]
        self.chat_messages[last_index + 1] = msg
        ...
        if next_server_id in self.all_servers.keys():
            self.udp_socket.sendto(('chat_msg_server ' + msg).encode(), self.all_servers[next_server_id])
            # TODO: insert your code here!
            #pass
    ...
    def recv_chat_msg_server(self, msg, sender):
        ...
        # TODO: insert your code here!
        for key, val in self.all_servers.items():
            if val == sender:
                sender_id = key
        
        if sender_id != self.server_id - 1:
            return 
    ...
    def process_pir_request(self, bitmask, sender):
        ...
        for index, bit in enumerate(bitmap_bits):
            if bit == "1":
                selected_messages.append(self.chat_messages[index])

        #print(selected_messages)
        if selected_messages != []:
            result = selected_messages[0]
            for i in range(1,len(selected_messages)):
                current = selected_messages[i]
        ...
        self.udp_socket.sendto(result.encode(), sender)
